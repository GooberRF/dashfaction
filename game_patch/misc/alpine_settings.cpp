...
bool alpine_player_settings_load(rf::Player* player)
{
    ...
    if (settings.count("SpectateShowCameras")) {
    ...
    if (settings.count("ControllerWarningShown")) {
        g_alpine_game_config.controller_warning_shown = std::stoi(settings["ControllerWarningShown"]);
        processed_keys.insert("ControllerWarningShown");
    }
    ...
    if (settings.count("GamepadCameraSensitivity")) {
    ...
    if (settings.count("GamepadGyroModifierMode")) {
        g_alpine_game_config.gamepad_gyro_ratcheting = std::clamp(std::stoi(settings["GamepadGyroModifierMode"]), 0, 5);
    ...
    if (settings.count("GamepadJoyCamera")) {
        g_alpine_game_config.gamepad_joy_camera = std::stoi(settings["GamepadJoyCamera"]) != 0;
        processed_keys.insert("GamepadJoyCamera");
    }
    if (settings.count("GamepadFlickstickAllowScoped")) {
        g_alpine_game_config.gamepad_flickstick_allow_scoped = std::stoi(settings["GamepadFlickstickAllowScoped"]) != 0;
        processed_keys.insert("GamepadFlickstickAllowScoped");
    }
    ...
    if (settings.count("GamepadRumbleVibrationFilter")) {
        g_alpine_game_config.gamepad_rumble_vibration_filter = std::clamp(std::stoi(settings["GamepadRumbleVibrationFilter"]), 0, 2);
        processed_keys.insert("GamepadRumbleVibrationFilter");
    }
    if (settings.count("GamepadRumbleWhenPrimary")) {
    ...
    if (settings.count("GamepadGyroScopeSensitivityModifier")) {
        g_alpine_game_config.set_gamepad_scope_gyro_sens_mod(std::stof(settings["GamepadGyroScopeSensitivityModifier"]));
        processed_keys.insert("GamepadGyroScopeSensitivityModifier");
    }
    if (settings.count("GamepadGyroScannerSensitivityModifier")) {
        ...
        if (key.rfind("Bind:", 0) == 0) {
            ...
            if (std::getline(bind_values, id_str, ',') &&
                ...
                if (bind_id >= 0 && bind_id < player->settings.controls.num_bindings) {
                    ...
                    // Optional 5th field: primary gamepad scan code
                    ...
                    if (std::getline(bind_values, gp_sc_str, ',') && !gp_sc_str.empty()) {
                        ...
                        else if (gp_sc == CTRL_GAMEPAD_LEFT_TRIGGER)
                            gamepad_set_trigger_action(0, bind_id);
                        else if (gp_sc == CTRL_GAMEPAD_RIGHT_TRIGGER)
    ...
    rf::console::printf("Successfully loaded settings from %s", filename.c_str());
...
void alpine_control_config_serialize(std::ofstream& file, const rf::ControlConfig& cc)
{
    ...
    file << "GamepadFlickstickAllowScoped=" << g_alpine_game_config.gamepad_flickstick_allow_scoped << "\n";
    ...
    file << "GamepadGyroRatchetingMode=" << g_alpine_game_config.gamepad_gyro_ratcheting << "\n";
...
void alpine_player_settings_save(rf::Player* player)
{
    ...
    file << "ControllerWarningShown=" << g_alpine_game_config.controller_warning_shown << "\n";
